<?php
    ...
    if($k['product_id'] != $p['product_id']){
      ...
      $insertcart = mysqli_query($conn, "INSERT INTO tb_keranjang(customer_id, customer_name, product_id, product_name, keranjang_status,qty) SELECT'".$c['customer_id']."','".$c['customer_name']."','".$p['product_id']."','".$p['product_name']."', 1,0");

      if($insertcart){
        echo '<script>alert("Silahkan Checkout Barang")</script>';
