      ...
      <div class="conteiner">
            ...
            <div class="row mt-5 ">
                <table  border="3" cellspacing="0" class = "table ms-4">
                    ...
                    <tbody>
                        <?php
                            $wishlist = mysqli_query($conn, "SELECT * FROM tb_order LEFT JOIN tb_customer USING (customer_id) LEFT JOIN tb_product USING (product_id) WHERE order_id = '".$_GET['id']."' and  customer_id ='".$_SESSION['id']."'");
                            $no = 1;
                        ...
                        <tr>
                            ...
                            <td>Rp. <?php echo number_format($row['product_price'] * $row['qty'] + 15000 + $row['order_id'] )?></td>
                        </tr>
                    </tbody>
                </table>
            </div>
          ...
          <div class="container">
            <div class="row">
              ...
              if(isset($_POST['submit'])){
                ...
                $update_payment = mysqli_query($conn, "UPDATE tb_payment SET payment_status = '".$status."',payment_proof = '".$p['payment_proof']."' WHERE order_id = '".$_GET['id']."'  ");
                $update_order = mysqli_query($conn, "UPDATE tb_order SET order_status = 2  WHERE order_id = '".$_GET['id']."'  ");
                if($update_payment && $update_order){
